...
public class OnTestClickListener implements OnClickListener {
	...
	private Uri buildUri(String scheme, String authority) {
		Log.e(P_TAG, "Called BUILD_URI FROM CONTENTPROVIDER");
	...
	private ContentValues[] initTestValues() {
		Log.e(P_TAG, "Called INIT_TEST_VALUES FROM CONTENTPROVIDER");
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		private boolean testInsert() {
			Log.e(P_TAG, "Called TEST_INSERT FROM CONTENTPROVIDER");
			try {
				for (int i = 0; i < TEST_CNT; i++) {
		...
		private boolean testQuery() {
			Log.e(P_TAG, "Called TEST_QUERY FROM CONTENTPROVIDER");
			try {
				for (int i = 0; i < TEST_CNT; i++) {
